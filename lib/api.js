 ...
 *
 * @param {string} coin - 'ltz' (default) or 'bch'
...
API.prototype.getFeeLevels = function(coin, network, cb) {
  ...
  $.checkArgument(coin || _.includes(['ltz', 'bch'], coin));
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  var coin = opts.coin || 'ltz';
  if (!_.includes(['ltz', 'bch'], coin)) return cb(new Error('Invalid coin'));
...
API.prototype.getBalance = function(opts, cb) {
  ...
  if (opts.coin) {
    if (!_.includes(['ltz', 'bch'], opts.coin)) return cb(new Error('Invalid coin'));
